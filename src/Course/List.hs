...
filter ::
  ...
  -> List a
filter _ Nil = Nil
filter pred (head :. tail) = if (pred(head)) then (head :. filter pred tail) else (filter pred tail)
...
flatten ::
  List (List a)
  -> List a
flatten = foldRight (\la acc -> la ++ acc) Nil
...
flattenAgain ::
  List (List a)
  -> List a
flattenAgain as = flatMap (\aa -> aa) as
...
seqOptional (Full item :. tail) = mapOptional (item :.) (seqOptional tail)
...
find ::
  ...
  -> Optional a
find _ Nil = Empty
find pred (head :. tail) = if (pred(head)) then (Full head) else find pred tail
...
-- Not sure what they are looking for here. It trivially seems possible as shown above.
